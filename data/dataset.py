...
from utils import MATERIAL_TO_CLASS, E_MEAN, E_STD, NU_MEAN, NU_STD
...
class Dataset(Dataset):
            ...
            if os.path.exists(phys_json_path):
                with open(phys_json_path, "r") as f:
                    ...
                    try:
                        ...
                        print(
                            f"Warning: Cannot convert E value '{E_raw}' to float, using 0.0"
                        )
                        E = 0.0
                    ...
                    try:
                        ...
                        print(
                            f"Warning: Cannot convert nu value '{nu_raw}' to float, using 0.0"
                        )
                        nu = 0.0

                    if material is not None:
                        physical_class = MATERIAL_TO_CLASS.get(material, 0)
                        return physical_class, E, nu
                    else:
                        print(
                            f"Warning: 'material' field not found in {phys_json_path}"
                        )
                        return 0
            else:
    ...
    def process_frames(self, frames, image_base_dir, random_crop_ratio=None):
        ...
        images = [
            Image.open(os.path.join(image_base_dir, frame["file_path"])).convert("RGBA")
            for frame in frames
        ]
        images = np.stack(
            [np.array(image) for image in images]
        )  # (num_frames, H, W, 4) 或 (num_frames, H, W, 3)
        ...
        images = np.stack(
            [cv2.resize(image, (resize_w, resize_h)) for image in images]
        )  # (num_frames, resize_h, resize_w, 3)
        start_h, start_w = 0, 0  
        if square_crop:
        ...
        )  # (num_frames, 4, resize_h, resize_w)
        ...
        if random_crop < 1.0:
            random_crop_ratio = (
                np.random.uniform(random_crop, 1.0)
                if random_crop_ratio is None
                else random_crop_ratio
            )
            magnify_ratio = 1.0 / random_crop_ratio
            cur_h, cur_w = images.shape[2], images.shape[3]
            images = F.interpolate(
                images, scale_factor=magnify_ratio, mode="bilinear", align_corners=False
            )
    ...
    def __getitem__(self, idx):
        try:
            ...
            # Load physical class from phys.json
            physical_class, E, nu = self.load_physical_class(image_base_dir)
            E_log = math.log10(max(E, 1.0))
            E_norm = (E_log - E_MEAN) / E_STD
            nu_norm = (nu - NU_MEAN) / NU_STD
            ...
            input_frame_idx = np.random.choice(
                frame_idx, num_input_frames, replace=False
            )

            # get target frames
            if target_has_input:
                base_target_idx = input_frame_idx.copy()
                additional_candidates = [
                    x for x in frame_idx if x not in base_target_idx
                ]
                additional_idx = np.random.choice(
                    additional_candidates,
                    num_target_frames - num_input_frames,
                    replace=False,
                )
                target_frame_idx = list(
                    set(np.concatenate([base_target_idx, additional_idx]))
                )
            else:
                target_frame_idx = np.random.choice(
                    frame_idx, num_target_frames, replace=False
                )

            random_crop_ratio = None
            target_frames = [frames[i] for i in target_frame_idx]
            target_images, target_intr, target_c2ws, random_crop_ratio = (
                self.process_frames(target_frames, image_base_dir)
            )

            input_frames = [frames[i] for i in input_frame_idx]
            input_images, input_intr, input_c2ws, _ = self.process_frames(
                input_frames, image_base_dir, random_crop_ratio
            )
            ...
            pos_avg = torch.stack(
                [right_avg, down_avg, forward_avg, position_avg], dim=1
            )  # (3, 4)
            pos_avg = torch.cat(
                [pos_avg, torch.tensor([[0, 0, 0, 1]], device=pos_avg.device).float()],
                dim=0,
            )  # (4, 4)
            ...
            ret_dict = {
                ...
                "physical_class": torch.tensor(physical_class).long(),
                "E": torch.tensor(E_norm).float(),  # Young's modulus
                "nu": torch.tensor(nu_norm).float(),  # Poisson's ratio
            }
        except Exception as e:
            traceback.print_exc()
            print(f"error loading data: {self.data_path[idx]}. Error: {e}")
...
if __name__ == "__main__":
    ...
    config.data.resize_h = 512
    config.data.resize_w = 512
    ...
    for i in range(len(dataset)):
        ...
        print("physical_class:", data["physical_class"])
        print("E (Young's modulus):", data["E"].item())
        print("nu (Poisson's ratio):", data["nu"].item())
